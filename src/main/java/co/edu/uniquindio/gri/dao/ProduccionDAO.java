...
import co.edu.uniquindio.gri.CrudRepository.ProduccionBCrudRepository;
...
import co.edu.uniquindio.gri.service.api.ProduccionServiceApi;
 ...
 */
@Service
public class ProduccionDAO extends GenericServiceImpl<ProduccionBGrupo, Long> implements ProduccionServiceApi {
	...
	/** Repository para producciones b grupo. */
	@Autowired
	ProduccionBCrudRepository produccionBCrudRepository;
	...
	public List getProduccionesConsolidado(String type, Long entityId, Long tipoId, String anio) {
		int starYear = Integer.parseInt(anio.split("-")[0]);
		int endYear = Integer.parseInt(anio.split("-")[1]);
		List respuesta = getProducciones(type, entityId, tipoId);
		List result = new ArrayList<>();

		for (int i = 0; i < respuesta.size(); i++) {

			if (respuesta.get(i) instanceof ProduccionGrupo) {
				ProduccionGrupo p = (ProduccionGrupo) respuesta.get(i);
				if (p.getAnio().chars().allMatch(Character::isDigit)) {
					int pAnio = Integer.parseInt(p.getAnio());
					if (pAnio >= starYear && pAnio <= endYear) {
						result.add(getProduccion(p.getId()));
					}
				}

			} else if (respuesta.get(i) instanceof ProduccionBGrupo) {
				ProduccionBGrupo p = (ProduccionBGrupo) respuesta.get(i);
				if (p.getAnio().chars().allMatch(Character::isDigit)) {
					int pAnio = Integer.parseInt(p.getAnio());
					if (pAnio >= starYear && pAnio <= endYear) {
						result.add(getProduccionB(p.getId()));
					}
				}
			}
		}
	...
	/**
	 * Obtiene la producción bibliográfica que coincida con el identificador de la
	 * revista.
	 *
	...
	public List<ProduccionBGrupo> getProduccionRevista(String identificador) {
		return produccionRepository.getProduccionBRevista(identificador);
	}

	/**
	 * Obtiene la cantidad producción bibliográfica que tengan una revista asociada
	 *
	 * @return numero de producciones bibliográficas con revista asociada
	 */
	public int getCantidadProduccionConRevista() {
		return produccionRepository.getCantidadProduccionConRevista();
	}

	@Override
	public CrudRepository<ProduccionBGrupo, Long> getCrudRepository() {
		return produccionBCrudRepository;
	}
