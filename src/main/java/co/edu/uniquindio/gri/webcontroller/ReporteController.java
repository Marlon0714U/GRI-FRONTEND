...
public class ReporteController {
	 ...
	 */
	@GetMapping("reporte")
	public String getReporte(@RequestParam(name = "prod", required = false, defaultValue = "0") String tipoProduccion,
		...
		model.addAttribute("producciones",
				produccionDAO.getProducciones(type, Long.parseLong(id), Long.parseLong(tipoProduccion)));
	...
	@GetMapping("reporteConsolidado")
	public String getReporteConsolidado(
		@RequestParam(name = "prod", required = false, defaultValue = "0") String tipoProduccion,
		@RequestParam(name = "type", required = false, defaultValue = "u") String type,
		@RequestParam(name = "id", required = false, defaultValue = "0") String id, String anio, Model model) {

		String[] producciones = tipoProduccion.split("-");
		String[] tipos = tipologiasController.getDatos(id, type);

		model.addAttribute("esBibliografica", true);
		model.addAttribute("id", id);
		model.addAttribute("tipo", type);
		model.addAttribute("nombre", tipos[0]);
		model.addAttribute("color", tipos[1]);
		model.addAttribute("facultadId", tipos[2]);

		List respuesta = new ArrayList<>();
		for (int i = 0; i < producciones.length; i++) {
			respuesta.addAll(produccionDAO.getProduccionesConsolidado(type, Long.parseLong(id),
					Long.parseLong(producciones[i]), anio));
		}

		model.addAttribute("tipo", "CONSOLIDADO");
		model.addAttribute("producciones", respuesta);

		return "reporteConsolidado";
	}

	/**
	 * Mapea la vista destinada a la visualización de resultados respecto a una
	 * cadena de búsqueda.
	 *
	 * @param type   el tipo de entidad que posee la producción: g en caso de
	 *               grupos, i en caso de investigadores.
