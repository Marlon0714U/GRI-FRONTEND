...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable()
				.authorizeRequests().antMatchers("/css/**", "/js/**", "/img/**", "/webjars/**").permitAll()
				.antMatchers("/inventario", "/reporteinventario", "/pertenencia", "/reportepertenencia").access("hasRole('ROLE_ADMIN')")
				.antMatchers("/general?id=6&type=f").access("hasRole('ROLE_USER')")
				.anyRequest().authenticated()
				.and().formLogin().loginPage("/login")
				.permitAll().defaultSuccessUrl("/").failureUrl("/login?error").usernameParameter("username")
...
//	}
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication().
			...
			withUser("inge").password(passwordEncoder().encode("password")).roles("INGE");
	}
